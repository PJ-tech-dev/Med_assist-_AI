...
@pytest.mark.asyncio
async def test_create_and_list_medicine_order(auth_headers):
    ...
    async with AsyncClient(transport=transport, base_url="http://test") as client:
        ...
        # 2. Get order by ID
        get_res = await client.get(f"/api/v1/orders/{order_id}", headers=auth_headers)
        ...
        patch_res = await client.patch(f"/api/v1/orders/{order_id}/status", json={"status": "OUT_FOR_DELIVERY"}, headers=auth_headers)
        assert patch_res.status_code == 200
        assert patch_res.json()["status"] == "OUT_FOR_DELIVERY"

        # 4. List orders
        list_res = await client.get("/api/v1/orders", headers=auth_headers)
